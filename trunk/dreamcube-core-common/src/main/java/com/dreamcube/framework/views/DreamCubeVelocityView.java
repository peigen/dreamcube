...
public class DreamCubeVelocityView extends VelocityLayoutView {
	 ...
	 * @param response
	 */
	private void renderModuleMergedTemplateModel(Map model,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		/** 在暴露了各种工具之后，需要将response也暴露到 */
		exposeHelpers(model, request, response);
...
//						writer);
	 ...
	 * @param response
	 */
	private void exposeHelpers(Map model, HttpServletRequest request,
	...
	protected void exposeHelpers(Map model, HttpServletRequest request)
		...
		Iterator<Entry<String, BeanTool>> iterator = beanToolMap.entrySet()
				.iterator();
		...
		for (; iterator.hasNext();) {
			...
			if (model.get("responseTempKey") != null) {
				HttpServletResponse response = (HttpServletResponse) model
						.get("responseTempKey");
