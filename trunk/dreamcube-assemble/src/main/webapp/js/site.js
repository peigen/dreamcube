...
function pageSlider(totalPage,currentPage,totalCount,perPage,rollpage,formName,pageControl,imgUrlPrex){
    try{
	  ...
	  if ((perPage == '') ||  isNaN(perPage)){
	    ...
	    if (preRollPage > 0){
	        txt = txt + "<a href=\"#?page=1\"><img src=\""+ imgUrlPrex + "/arrow4-1.gif\" border=\"0\" alt=\"首页\" onclick=\"findObj('" + pageControl + "').value=1; " + formName + ".submit(); \"></img></a>";
	        txt = txt + "&nbsp;";
	        txt = txt + "<a href=\"#?page=" + preRollPage + "\"><img src=\""+ imgUrlPrex + "/arrow3-1.gif\" border=\"0\" alt=\"前一页\" onclick=\"findObj('" + pageControl + "').value="+preRollPage+"; " + formName + ".submit(); \" align=absmiddle></img></a>";
	    }else{
	    ...
	    for(i = 1; i <= rollpage; i++){
	        ...
	        if (curPage <= totalPage){
	            if (curPage != currentPage){
	                txt = txt + "<a href=\"#?page=" +curPage+ "\" onclick=\"findObj('" + pageControl + "').value=" +curPage+ "; " + formName + ".submit(); \">" +curPage+ "</a>";
	                txt = txt + "&nbsp;";
	            }else{
	    ...
	    if ( nextRollPage <= totalPage ){
	        txt = txt + "<a href=\"#?page=" + nextRollPage + "\" onclick=\"findObj('" + pageControl + "').value=" +nextRollPage+ "; " + formName + ".submit(); \"><img src=\""+ imgUrlPrex + "/arrow1-1.gif\" border=\"0\" alt=\"后一页\"></img></a>";
	        txt = txt + "&nbsp;";
	        txt = txt + "<a href=\"#?page=" + totalPage + "\" onclick=\"findObj('" + pageControl + "').value=" +totalPage+ "; " + formName + ".submit(); \"><img src=\""+ imgUrlPrex + "/arrow2-1.gif\" border=\"0\" alt=\"尾页\" align=absmiddle></img></a>";
	    }else{
